...
`sudo apt install valgrind`     - установка утилиты для отладки и обнаружения утечек памяти;
`valgrind --leack-check=full ./main`    - запуск проверки программы
...
[Стандартная библиотека языка C](https://manual.cs50.io/)
...
<b>Значение указателя изменяется на <em>размер типа данных</em>, для которого он определен.</b>
...
printf("%u\n", ptr_f);
```
Первый вывод: <b>898233836</b>
Второй вывод: <b>898233856</b>
Тип указателя int, int занимает 4 байта, 5 * 4 = 20. Значение адреса изменяется на 20.
...
`sizeof` выводит размер переменной или типа в байтах.
